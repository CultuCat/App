...
const ChatScreen = ({ user, userLId, onBack}) => {
  ...
  /*const initializePusher = async () => {
    try {
      ...
      channel.bind('message', data => {
        if (
          (data.user_from === uId && data.user_to === uIdR) ||
          (data.user_from === uIdR && data.user_to === uId)
        ) {
  ...
  useEffect(() => {
    const initializePusher = async () => {
      ...
      const pusher = new Pusher('023f0639c639ffe33066', {
        cluster: 'eu',
        encrypted: true, // Consider setting this based on your configuration
      });
      ...
      channel.bind('message', data => {
        if (
          ...
          // Assuming you're using state to manage messages
          setMessages(prevMessages => [...prevMessages, data]);
        }
      });
    };
    ...
    return () => {
      // Unsubscribe or perform cleanup when the component unmounts
    };
  }, [uId, uIdR]);
